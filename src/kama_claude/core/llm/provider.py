...
# 规范化第三方兼容端点，避免 SDK 再次追加 /v1 时产生重复路径
def _normalize_base_url(base_url: str) -> str:
    ...
    # DeepSeek 的 Anthropic 兼容接口位于 /anthropic/v1/messages。
    if host == "api.deepseek.com" and path in ("", "/v1"):
        return urlunsplit((parts.scheme, parts.netloc, "/anthropic", parts.query, parts.fragment))

    # 允许用户直接提供 /anthropic/v1，统一归一化为 /anthropic。
    if host == "api.deepseek.com" and path == "/anthropic/v1":
        return urlunsplit((parts.scheme, parts.netloc, "/anthropic", parts.query, parts.fragment))
